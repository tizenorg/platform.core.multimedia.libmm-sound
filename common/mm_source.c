...
EXPORT_API int mm_source_open_file(const char *filename, MMSourceType * source, int drmsupport)
{
	struct stat finfo = { 0, };
	int fd = -1;
	void *mmap_buf = NULL;
...
EXPORT_API int mm_source_open_full_memory(const void *ptr, int totsize, int alloc, MMSourceType * source)
{
...
EXPORT_API int mm_source_open_memory(const void *ptr, int totsize, int size, MMSourceType * source)
{
...
EXPORT_API int mm_source_append_memory(const void *ptr, int size, MMSourceType * source)
{
...
EXPORT_API int mm_source_close(MMSourceType * source)
{
	if (source == NULL) {
