...
int MMSoundPlugCodecOggCreate(mmsound_codec_param_t * param, mmsound_codec_info_t * info, MMHandleType * handle)
{
	...
	debug_msg
		("[CODEC OGG] PARAM mode:[%d] priority:[%d] policy:[%d] channels:[%d] samplerate:[%d] format:[%d] volume type:[%x]\n",
		 mode, -1, route_info.policy, ss.channels, ss.rate, ss.format, param->volume_config);

	p->handle =
		mm_sound_pa_open(HANDLE_MODE_OUTPUT_LOW_LATENCY, &route_info, 0,
						 param->volume_config, &ss, NULL, &size, param->stream_type, param->stream_index);
	if (!p->handle) {
