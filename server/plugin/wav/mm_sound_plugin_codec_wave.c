...
//  struct __fmt_chunk *pfmt = NULL;
	...
	if (priff->chunkid != RIFF_CHUNK_ID || priff->chunksize != datalen - 8 || priff->rifftype != RIFF_CHUNK_TYPE) {
		...
		debug_msg
			("[CODEC WAV] cunkid : %ld, chunksize : %ld, rifftype : 0x%lx\n",
	...
	if (pwav->chunkid != FMT_CHUNK_ID || pwav->compression != WAVE_CODE_PCM ||	/* Only supported PCM */
		...
		debug_msg
			("[CODEC WAV] chunkid : 0x%lx, comp : 0x%x, av byte/sec : %lu, blockalign : %d\n",
...
int MMSoundPlugCodecWaveCreate(mmsound_codec_param_t * param, mmsound_codec_info_t * info, MMHandleType * handle)
{
	...
	debug_msg
		("period[%d] type[%s] ch[%d] format[%d] rate[%d] doffset[%d] priority[%d] repeat[%d] volume[%f] callback[%p] route[%d]\n",
		 keytone_period,
		 (info->codec == MM_SOUND_SUPPORTED_CODEC_WAVE) ? "Wave" : "Unknown",
		 info->channels, info->format, info->samplerate, info->doffset,
		 param->priority, param->repeat_count, param->volume, param->stop_cb, param->handle_route);
	source = param->source;
...
static void _runing(void *param)
{
	...
	p->handle =
		mm_sound_pa_open(p->mode, &route_info, p->priority, p->volume_config,
						 &ss, NULL, &size, p->stream_type, p->stream_index);
	if (!p->handle) {
	...
	while (((p->repeat_count == -1) ? (1) : (p->repeat_count--))
		   && p->state == STATE_PLAY) {
