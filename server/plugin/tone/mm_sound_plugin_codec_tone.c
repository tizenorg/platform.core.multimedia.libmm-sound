...
int MMSoundPlugCodecToneCreate(mmsound_codec_param_t * param, mmsound_codec_info_t * info, MMHandleType * handle)
{
	...
	toneInfo->handle =
		mm_sound_pa_open(HANDLE_MODE_OUTPUT, &route_info,
						 HANDLE_PRIORITY_NORMAL, param->volume_config, &ss,
						 NULL, &size, param->stream_type, param->stream_index);
	if (!toneInfo->handle) {
...
static char *_create_tone(double *sample, TONE _TONE, double volume, int *toneSize, gboolean is_cycle_ended)
{
	short *pbuf;
	double i = 0;
...
static TONE _mm_get_tone(int key, int CurIndex)
{
	TONE _TONE;
...
static int _mm_get_waveCnt_PlayingTime(int toneTime, TONE _TONE, int *waveCnt, int *waveRestPlayTime)
{
...
static int _mm_get_CurIndex(TONE _TONE, int *CurArrayPlayCnt, int *CurIndex)
{
	int ret = MM_ERROR_NONE;
...
static void _running_tone(void *param)
{
	...
	while (1) {
		...
		debug_log
			("Predefined Tone[%d] Total Play time (ms) : %d, _TONE.playing_time: %d _numWave = %d low_frequency: %0.f, middle_frequency: %0.f, high_frequency: %0.f\n",
		...
		for (CurWaveIndex = 0; CurWaveIndex < numWave + 1; CurWaveIndex++) {
			...
			if (prePlayingTime == toneTime || toneInfo->state != STATE_PLAY) {
				debug_log
					("Finished.....on Total Playing Time : %d _TONE.playing_time: %d\n", prePlayingTime, _TONE.playingTime);
