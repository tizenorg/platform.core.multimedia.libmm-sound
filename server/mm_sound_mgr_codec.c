...
void sound_codec_focus_signal_callback(mm_sound_signal_name_t signal, int value, void *user_data)
{

	int slotid = (int)user_data;
	int result = MM_ERROR_NONE;

	debug_warning ("focus signal callback is called -> value(%d)", value);

	if(value == 1) {
		if(g_slots[slotid].focus_handle) {
			if(g_slots[slotid].session_type != ASM_EVENT_CALL &&
				g_slots[slotid].session_type != ASM_EVENT_VIDEOCALL &&
				g_slots[slotid].session_type != ASM_EVENT_VOIP &&
				g_slots[slotid].session_type != ASM_EVENT_VOICE_RECOGNITION &&
				g_slots[slotid].enable_session ) {
				if ((g_slots[slotid].session_options & ASM_SESSION_OPTION_PAUSE_OTHERS) || g_slots[slotid].session_type == ASM_EVENT_ALARM || g_slots[slotid].session_type == ASM_EVENT_NOTIFY || g_slots[slotid].session_type == ASM_EVENT_EMERGENCY) {
					result = mm_sound_release_focus(g_slots[slotid].focus_handle, FOCUS_FOR_BOTH, NULL);
					if (result) {
						debug_error("mm_sound_release_focus failed [0x%x]", result);
					}
				} else if (!(g_slots[slotid].session_options & ASM_SESSION_OPTION_PAUSE_OTHERS)) {
					result = mm_sound_unset_focus_watch_callback(g_slots[slotid].focus_wcb_id);
					if (result) {
						debug_error("mm_sound_unset_focus_watch_callback failed [0x%x]", result);
					}
				}

				if(!mm_sound_unregister_focus(g_slots[slotid].focus_handle) || result) {
...
int MMSoundMgrCodecPlay(int *slotid, const mmsound_mgr_codec_param_t *param)
{
	...
	err = mm_sound_subscribe_signal(MM_SOUND_SIGNAL_RELEASE_INTERNAL_FOCUS, &focus_sig_id, sound_codec_focus_signal_callback, (void*)*slotid);
	if (err) {
		debug_error("mm_sound_subscribe_signal failed [0x%x]", err);
		pthread_mutex_unlock(&g_slot_mutex);
		return MM_ERROR_POLICY_INTERNAL;
	}
	err = mm_sound_get_signal_value(MM_SOUND_SIGNAL_RELEASE_INTERNAL_FOCUS, &is_focus_registered);
	if (err) {
		debug_error("mm_sound_subscribe_signal failed [0x%x]", err);
		pthread_mutex_unlock(&g_slot_mutex);
		return MM_ERROR_POLICY_INTERNAL;
	...
	if (param->session_type != ASM_EVENT_CALL &&
		...
		if ((param->session_options & ASM_SESSION_OPTION_PAUSE_OTHERS) || param->session_type == ASM_EVENT_ALARM || param->session_type == ASM_EVENT_NOTIFY || param->session_type == ASM_EVENT_EMERGENCY) {
	...
	g_slots[*slotid].focus_sig_id = focus_sig_id;
...
static int _MMSoundMgrCodecStopCallback(int param)
{
	...
	if (g_slots[param].focus_handle) {
		if(g_slots[param].session_type != ASM_EVENT_CALL &&
			...
			if ((g_slots[param].session_options & ASM_SESSION_OPTION_PAUSE_OTHERS) || g_slots[param].session_type == ASM_EVENT_ALARM || g_slots[param].session_type == ASM_EVENT_NOTIFY || g_slots[param].session_type == ASM_EVENT_EMERGENCY) {
	...
	mm_sound_unsubscribe_signal(g_slots[param].focus_sig_id);
