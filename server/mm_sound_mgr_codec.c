...
void sound_codec_focus_callback(int id, mm_sound_focus_type_e focus_type,
								mm_sound_focus_state_e focus_state,
								const char *reason_for_change, const char *additional_info, void *user_data)
{
	...
	debug_warning
		("focus callback called -> focus_stae(%d), reasoun_for_change(%s)",
...
void sound_codec_focus_watch_callback(int id, mm_sound_focus_type_e focus_type,
									  mm_sound_focus_state_e focus_state,
									  const char *reason_for_change, const char *additional_info, void *user_data)
{
	int slotid = (int)user_data;
...
int MMSoundMgrCodecPlay(int *slotid, const mmsound_mgr_codec_param_t * param)
{
	...
	if (param->session_type != MM_SESSION_TYPE_CALL
		...
		&& param->session_type != MM_SESSION_TYPE_VOICE_RECOGNITION && param->enable_session) {

		unsigned int subs_id = 0;

		err = mm_sound_add_device_connected_callback(MM_SOUND_DEVICE_STATE_ACTIVATED_FLAG, (mm_sound_device_connected_cb)
													 sound_codec_device_connected_callback, *slotid, &subs_id);
		if (err) {
		...
		if ((param->session_options & MM_SESSION_OPTION_PAUSE_OTHERS)
			...
			|| param->session_type == MM_SESSION_TYPE_NOTIFY || param->session_type == MM_SESSION_TYPE_EMERGENCY) {
			debug_warning("session option is PAUSE_OTHERS -> acquire focus");
			err = mm_sound_focus_get_id((int *)(&param->focus_handle));
			err =
				mm_sound_register_focus_for_session(param->focus_handle,
													(int)param->param, "media", sound_codec_focus_callback, (void *)*slotid);
			if (err) {
			...
			err =
				mm_sound_set_focus_watch_callback_for_session((int)param->param,
															  FOCUS_FOR_BOTH,
															  sound_codec_focus_watch_callback,
 ...
 cleanup:
	if (param->session_type != MM_SESSION_TYPE_CALL
		&& param->session_type != MM_SESSION_TYPE_VIDEOCALL
		&& param->session_type != MM_SESSION_TYPE_VOIP
		&& param->session_type != MM_SESSION_TYPE_VOICE_RECOGNITION && param->enable_session && need_focus_unregister == 1) {
		...
		if (g_slots[slotid].pid == pid) {
			if (g_slots[slotid].focus_handle || g_slots[slotid].focus_wcb_id) {
				if (g_slots[slotid].session_type != MM_SESSION_TYPE_CALL
					...
					&& g_slots[slotid].session_type != MM_SESSION_TYPE_VOICE_RECOGNITION && g_slots[slotid].enable_session) {
					if ((g_slots[slotid].session_options & MM_SESSION_OPTION_PAUSE_OTHERS)
						|| g_slots[slotid].session_type == MM_SESSION_TYPE_ALARM
						|| g_slots[slotid].session_type ==
						MM_SESSION_TYPE_NOTIFY || g_slots[slotid].session_type == MM_SESSION_TYPE_EMERGENCY) {
...
static int _MMSoundMgrCodecStopCallback(int param)
{
	...
	if (g_slots[param].focus_handle || g_slots[param].focus_wcb_id) {
		if (g_slots[param].session_type != MM_SESSION_TYPE_CALL
			...
			&& g_slots[param].session_type != MM_SESSION_TYPE_VOICE_RECOGNITION && g_slots[param].enable_session) {
			if ((g_slots[param].session_options & MM_SESSION_OPTION_PAUSE_OTHERS)
				...
				if (mm_sound_unregister_focus(g_slots[param].focus_handle)
					|| err) {
					debug_error("Focus clean up failed [0x%x]", err);
					pthread_mutex_unlock(&g_slot_mutex);
					return MM_ERROR_POLICY_INTERNAL;
