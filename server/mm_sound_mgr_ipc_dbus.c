...
static const gchar introspection_xml[] =
	...
	"      <arg type='i' name='session_type' direction='in'/>"
	"      <arg type='i' name='session_option' direction='in'/>"
	"      <arg type='i' name='client_pid' direction='in'/>" "      <arg type='b' name='enable_session' direction='in'/>" "	     <arg type='s' name='stream_type' direction='in'/>" "	     <arg type='i' name='stream_index' direction='in'/>" "      <arg type='i' name='handle' direction='out'/>" "    </method>" "	   <method name='PlayDTMFWithStreamInfo'>" "	     <arg type='i' name='tone' direction='in'/>" "	     <arg type='i' name='repeat' direction='in'/>" "	     <arg type='i' name='volume' direction='in'/>" "	     <arg type='i' name='client_pid' direction='in'/>" "	     <arg type='s' name='stream_type' direction='in'/>" "	     <arg type='i' name='stream_index' direction='in'/>" "	     <arg type='i' name='handle' direction='out'/>" "	   </method>" "    <method name='GetConnectedDeviceList'>" "      <arg type='i' name='device_mask' direction='in'/>" "      <arg type='a(iiiis)' name='device_list' direction='out'/>" "    </method>" "  </interface>" "</node>";
...
/*
	...
	*/
	...
	emit_success = g_dbus_connection_emit_signal(conn_g, NULL, OBJECT_SOUND_SERVER, INTERFACE_SOUND_SERVER, signals[signal_type].info.name, parameter, &err);
	if (!emit_success && err) {
...
static int _get_sender_pid(GDBusMethodInvocation * invocation)
{
	...
	value = g_dbus_connection_call_sync(connection, "org.freedesktop.DBus", "/org/freedesktop/DBus", "org.freedesktop.DBus", "GetConnectionUnixProcessID", g_variant_new("(s)", sender, NULL), NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, &err);
	if (value) {
...
static void handle_method_play_file_start(GDBusMethodInvocation * invocation)
{
	...
	g_variant_get(params, "(siiiiiiiiibsi)", &filename, &tone, &repeat, &volume, &vol_config, &priority, &session_type, &session_option, &pid, &handle_route, &enable_session, &stream_type, &stream_index);
	if (!filename) {
		debug_error("filename null");
		ret = MM_ERROR_SOUND_INTERNAL;
		goto send_reply;
	}
	ret = _MMSoundMgrIpcPlayFile(filename, tone, repeat, volume, vol_config, priority, session_type, session_option, _get_sender_pid(invocation), handle_route, enable_session, &slotid, stream_type, stream_index);
...
static void handle_method_play_file_start_with_stream_info(GDBusMethodInvocation * invocation)
{
	...
	g_variant_get(params, "(siiiiisi)", &filename, &repeat, &volume, &priority, &pid, &handle_route, &stream_type, &stream_index);
	if (!filename) {
		debug_error("filename null");
		ret = MM_ERROR_SOUND_INTERNAL;
		goto send_reply;
	}
	ret = _MMSoundMgrIpcPlayFileWithStreamInfo(filename, repeat, volume, priority, _get_sender_pid(invocation), handle_route, &slotid, stream_type, stream_index);
...
static void handle_method_play_dtmf(GDBusMethodInvocation * invocation)
{
	...
	g_variant_get(params, "(iiiiiiibsi)", &tone, &repeat, &volume, &vol_config, &session_type, &session_option, &pid, &enable_session, &stream_type, &stream_index);
	debug_error("volume - %d", volume);
	ret = _MMSoundMgrIpcPlayDTMF(tone, repeat, volume, vol_config, session_type, session_option, _get_sender_pid(invocation), enable_session, &slotid, stream_type, stream_index);
...
static void handle_method_call(GDBusConnection * connection, const gchar * sender, const gchar * object_path, const gchar * interface_name, const gchar * method_name, GVariant * parameters, GDBusMethodInvocation * invocation, gpointer userdata)
{
	int method_idx = 0;
...
static GVariant *handle_get_property(GDBusConnection * connection, const gchar * sender, const gchar * object_path, const gchar * interface_name, const gchar * property_name, GError ** error, gpointer userdata)
{
	debug_log("Get Property, obj : %s, intf : %s, prop : %s", object_path, interface_name, property_name);
	return NULL;
}

static gboolean handle_set_property(GDBusConnection * connection, const gchar * sender, const gchar * object_path, const gchar * interface_name, const gchar * property_name, GVariant * value, GError ** error, gpointer userdata)
{
...
static void on_bus_acquired(GDBusConnection * connection, const gchar * name, gpointer user_data)
{
	...
	conn_g = connection;
	reg_id = g_dbus_connection_register_object(connection, OBJECT_SOUND_SERVER, introspection_data->interfaces[0], &interface_vtable, NULL, NULL, NULL);
	if (!reg_id) {
...
static int _mm_sound_mgr_dbus_own_name(GBusType bus_type, const char *wellknown_name, guint * owner_id)
{
	...
	oid = g_bus_own_name(bus_type, wellknown_name, G_BUS_NAME_OWNER_FLAGS_NONE, on_bus_acquired, on_name_acquired, on_name_lost, NULL, NULL);
	if (oid <= 0) {
...
int mm_sound_mgr_ipc_dbus_send_signal_freeze(char *command, int pid)
{
	...
	ret = g_dbus_connection_emit_signal(conn, NULL, PROC_DBUS_OBJECT, PROC_DBUS_INTERFACE, PROC_DBUS_METHOD, g_variant_new("(si)", command, pid), &err);
	if (!ret && err) {
...
int __mm_sound_mgr_ipc_dbus_get_stream_list(stream_list_t * stream_list)
{
	...
	result = g_dbus_connection_call_sync(conn, PA_BUS_NAME, PA_STREAM_MANAGER_OBJECT_PATH, PA_STREAM_MANAGER_INTERFACE, PA_STREAM_MANAGER_METHOD_NAME_GET_STREAM_LIST, NULL, G_VARIANT_TYPE("(vv)"), G_DBUS_CALL_FLAGS_NONE, 2000, NULL, &err);
	if (!result && err) {
