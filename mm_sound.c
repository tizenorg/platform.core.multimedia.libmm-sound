...
EXPORT_API int mm_sound_play_loud_solo_sound(const char *filename, int volume_config, mm_sound_stop_callback_func callback, void *data, int *handle)
{
...
EXPORT_API int mm_sound_play_sound(const char *filename, int volume_config, mm_sound_stop_callback_func callback, void *data, int *handle)
{
	MMSoundPlayParam param = { 0, };
...
EXPORT_API int mm_sound_play_sound_with_stream_info(const char *filename, char *stream_type, int stream_id, mm_sound_stop_callback_func callback, void *data, int *handle)
{
	MMSoundPlayParam param = { 0, };
	int err;
...
////     MMSOUND TONE APIs
///////////////////////////////////
EXPORT_API int mm_sound_play_tone_ex(MMSoundTone_t num, int volume_config, const double volume, const int duration, int *handle, bool enable_session)
{
	int lhandle = -1;
...
EXPORT_API int mm_sound_play_tone_with_stream_info(MMSoundTone_t tone, char *stream_type, int stream_id, const double volume, const int duration, int *handle)
{
...
static void signal_callback(GDBusConnection * conn, const gchar * sender_name, const gchar * object_path, const gchar * interface_name, const gchar * signal_name, GVariant * parameters, gpointer user_data)
{
	int value = 0;
...
EXPORT_API int mm_sound_subscribe_signal(mm_sound_signal_name_t signal, unsigned int *subscribe_id, mm_sound_signal_callback callback, void *user_data)
{
	int ret = MM_ERROR_NONE;
	GError *err = NULL;
	...
	*subscribe_id = g_dbus_connection_signal_subscribe(g_dbus_conn_mmsound, NULL, MM_SOUND_DBUS_INTERFACE, dbus_signal_name_str[signal], MM_SOUND_DBUS_OBJECT_PATH, NULL, 0, signal_callback, subscribe_cb, NULL);
	if (*subscribe_id == 0) {
...
EXPORT_API int mm_sound_send_signal(mm_sound_signal_name_t signal, int value)
{
	...
	dbus_ret = g_dbus_connection_emit_signal(conn, NULL, MM_SOUND_DBUS_OBJECT_PATH, MM_SOUND_DBUS_INTERFACE, dbus_signal_name_str[signal], g_variant_new("(i)", value), &err);
	if (!dbus_ret && err) {
