...
EXPORT_API int mm_sound_play_keysound(const char *filename, int volume_config)
{
...
static int _mm_sound_play_keysound(const char *filename, int volume_config, ipc_type_t ipc_type)
{
	...
	if (ipc_type == IPC_TYPE_PIPE) {
		...
		if (!conn && err) {
			...
			result = g_dbus_connection_call_sync(conn, PA_BUS_NAME, PA_SOUND_PLAYER_OBJECT_PATH, PA_SOUND_PLAYER_INTERFACE, PA_SOUND_PLAYER_METHOD_NAME_SIMPLE_PLAY, g_variant_new("(sss)", filename, role, vol_gain_type), NULL, G_DBUS_CALL_FLAGS_NONE, 2000, NULL, &err);
			if (!result && err) {
