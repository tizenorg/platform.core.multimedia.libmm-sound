		...
		if (!conn && err) {
			...
			result =
				g_dbus_connection_call_sync(conn, PA_BUS_NAME,
											...
											g_variant_new("(sss)", filename,
														  role, vol_gain_type), NULL, G_DBUS_CALL_FLAGS_NONE, 2000, NULL, &err);
			if (!result && err) {
				...
				if (idx == -1) {
					debug_error
						("SIMPLE_PLAY failure, filename(%s)/role(%s)/gain(%s)/stream idx(%d)",
						 filename, role, vol_gain_type, idx);
					ret = MM_ERROR_SOUND_INTERNAL;
				} else {
					debug_msg
						("SIMPLE_PLAY success, filename(%s)/role(%s)/gain(%s)/stream idx(%d)",
						 filename, role, vol_gain_type, idx);
				}
