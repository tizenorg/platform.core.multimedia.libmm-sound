...
mm_sound_focus_session_interrupt_cb g_focus_session_interrupt_user_cb = NULL;
...
static gboolean _focus_callback_handler(gpointer d)
{
	...
	if (data->revents & (POLLIN | POLLPRI)) {
		...
		if (changed_state != -1) {
			...
			if (g_focus_session_interrupt_user_cb) {
				debug_error("sending session interrupt callback(%p)", g_focus_session_interrupt_user_cb);
				(g_focus_session_interrupt_user_cb)(cb_data.state, cb_data.stream_type, false);
			}
		}
...
static gboolean _focus_watch_callback_handler( gpointer d)
{
	...
	if (data->revents & (POLLIN | POLLPRI)) {
		...
		if (g_focus_session_interrupt_user_cb) {
			debug_error("sending session interrupt callback(%p)", g_focus_session_interrupt_user_cb);
			(g_focus_session_interrupt_user_cb)(cb_data.state, cb_data.stream_type, true);
		}
...
void mm_sound_client_dbus_set_session_interrupt_callback(mm_sound_focus_session_interrupt_cb callback)
{
	...
	g_focus_session_interrupt_user_cb = callback;
...
int mm_sound_client_dbus_unset_session_interrupt_callback(void)
{
	...
	g_focus_session_interrupt_user_cb = NULL;
