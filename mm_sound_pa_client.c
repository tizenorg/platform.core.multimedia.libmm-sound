...
EXPORT_API int mm_sound_pa_open(MMSoundHandleMode mode,
								mm_sound_handle_route_info * route_info,
								MMSoundHandlePriority priority,
								int volume_config, pa_sample_spec * ss,
								pa_channel_map * channel_map, int *size, char *stream_type, int stream_index)
{
	pa_simple *s = NULL;
	pa_channel_map maps;
	pa_buffer_attr attr;
	...
	case HANDLE_MODE_INPUT:
		...
		s = pa_simple_new_proplist(NULL, "MM_SOUND_PA_CLIENT", PA_STREAM_RECORD,
								   NULL, "CAPTURE", ss, channel_map, &attr, proplist, &err);
		break;
	...
	case HANDLE_MODE_INPUT_LOW_LATENCY:
		...
		s = pa_simple_new_proplist(NULL, "MM_SOUND_PA_CLIENT", PA_STREAM_RECORD,
								   NULL, "LOW LATENCY CAPTURE", ss, channel_map, &attr, proplist, &err);
		break;
	...
	case HANDLE_MODE_INPUT_HIGH_LATENCY:
		...
		s = pa_simple_new_proplist(NULL, "MM_SOUND_PA_CLIENT", PA_STREAM_RECORD,
								   NULL, "HIGH LATENCY CAPTURE", ss, channel_map, &attr, proplist, &err);
		break;
	...
	case HANDLE_MODE_OUTPUT:
		...
		s = pa_simple_new_proplist(NULL, "MM_SOUND_PA_CLIENT",
								   PA_STREAM_PLAYBACK, NULL, "PLAYBACK", ss, channel_map, &attr, proplist, &err);
		break;
	...
	case HANDLE_MODE_OUTPUT_LOW_LATENCY:
		...
		debug_msg
			("prebuf(%d), minreq(%d), tlength(%d), maxlength(%d), fragsize(%d)",
			 attr.prebuf, attr.minreq, attr.tlength, attr.maxlength, attr.fragsize);

		s = pa_simple_new_proplist(NULL, "MM_SOUND_PA_CLIENT",
								   PA_STREAM_PLAYBACK, NULL, "LOW LATENCY PLAYBACK", ss, channel_map, &attr, proplist, &err);
		break;
	...
	case HANDLE_MODE_OUTPUT_CLOCK:
		...
		s = pa_simple_new_proplist(NULL, "MM_SOUND_PA_CLIENT",
								   PA_STREAM_PLAYBACK, NULL, "HIGH LATENCY PLAYBACK", ss, channel_map, &attr, proplist, &err);
		break;
	...
	case HANDLE_MODE_OUTPUT_AP_CALL:
		...
		s = pa_simple_new_proplist(NULL, "MM_SOUND_PA_CLIENT",
								   PA_STREAM_PLAYBACK, NULL, "VoIP PLAYBACK", ss, channel_map, &attr, proplist, &err);
#endif
		break;
	case HANDLE_MODE_INPUT_AP_CALL:
		...
		s = pa_simple_new_proplist(NULL, "MM_SOUND_PA_CLIENT", PA_STREAM_RECORD,
								   NULL, "VoIP CAPTURE", ss, channel_map, &attr, proplist, &err);
#endif
		break;
	...
	*size = handle->period;
	...
	debug_msg
		("created handle[%d]. mode(%d), policy(%d), volumetype(%d), gain(%d), rate(%d), channels(%d), format(%d), stream_idx(%d), s(%x), source_type(%d) handle_inout(%d)",
		 handle->handle, handle->mode, handle->policy, handle->volume_type,
		 handle->gain_type, handle->rate, handle->channels, ss->format,
		 handle->stream_idx, handle->s, handle->source_type, handle_inout);
...
EXPORT_API int mm_sound_pa_read(const int handle, void *buf, const int size)
{
...
EXPORT_API int mm_sound_pa_write(const int handle, void *buf, const int size)
{
	mm_sound_handle_t *phandle = NULL;
	int err = MM_ERROR_NONE;
...
#ifdef __STREAM_DEBUG__
	debug_msg
		("phandle(%x) s(%x), handle(%d), rate(%d), ch(%d) stream_idx(%d), buf(%p), size(%d)",
...
EXPORT_API int mm_sound_pa_close(const int handle)
{
	mm_sound_handle_t *phandle = NULL;
	int err = MM_ERROR_NONE;
...
EXPORT_API int mm_sound_pa_cork(const int handle, const int cork)
{
	mm_sound_handle_t *phandle = NULL;
	int err = MM_ERROR_NONE;
...
EXPORT_API int mm_sound_pa_drain(const int handle)
{
	mm_sound_handle_t *phandle = NULL;
	int err = MM_ERROR_NONE;
...
EXPORT_API int mm_sound_pa_flush(const int handle)
{
	mm_sound_handle_t *phandle = NULL;
	int err = MM_ERROR_NONE;
...
EXPORT_API int mm_sound_pa_get_latency(const int handle, int *latency)
{
	mm_sound_handle_t *phandle = NULL;
	int err = MM_ERROR_NONE;
...
EXPORT_API int mm_sound_pa_corkall(int cork)
{
	pa_operation *o = NULL;
	...
	o = pa_context_set_cork_all(mm_sound_handle_mgr.context, cork,
								__mm_sound_pa_success_cb, (void *)mm_sound_handle_mgr.mainloop);
