...
static void __set_container_data(int pid, int handle, const char *container_name, int container_pid)
{
	...
	for (list = g_focus_node_list; list != NULL; list = list->next) {
		...
		if (!node->is_for_watch && node->pid == pid && node->handle_id == handle) {
			debug_error("Set container [%s][%d] to handle[%d] instanceID[%d]", container_name, container_pid, handle, node->pid);
			if (container_name)
...
static int _mm_sound_mgr_focus_list_dump()
{
	...
	for (list = g_focus_node_list; list != NULL; list = list->next) {
		...
		if (node && !node->is_for_watch) {
			debug_log("*** pid[%5d]/handle_id[%d]/[%15s]:priority[%2d],status[%s],taken_by[P(%5d/%2d/%2d)C(%5d/%2d/%2d)],for_session[%d],add.info[%s/%s]\n", node->pid, node->handle_id, node->stream_type, node->priority, focus_status_str[node->status], node->taken_by_id[0].pid, node->taken_by_id[0].handle_id, node->taken_by_id[0].by_session, node->taken_by_id[1].pid, node->taken_by_id[1].handle_id, node->taken_by_id[1].by_session, node->is_for_session, node->option[0], node->option[1]);
		}
	}
...
int mm_sound_mgr_focus_request_acquire(const _mm_sound_mgr_focus_param_t * param)
{
	...
	for (list = g_focus_node_list; list != NULL; list = list->next) {
		...
		if (!node) {
			...
			/* skip */
		} else if (param->request_type == FOCUS_TYPE_BOTH || node->status == FOCUS_STATUS_ACTIVATED_BOTH || (node->status & param->request_type)) {
	...
	if (need_to_trigger_cb) {
		...
		for (list = g_focus_node_list; list != NULL; list = list->next) {
			...
			if (!node) {
				...
				/* skip */
			} else if (param_s->request_type == FOCUS_TYPE_BOTH || node->status == FOCUS_STATUS_ACTIVATED_BOTH || (node->status & param_s->request_type)) {
				if (node->status > FOCUS_STATUS_DEACTIVATED) {
					if (my_node->priority >= node->priority) {
...
int mm_sound_mgr_focus_request_release(const _mm_sound_mgr_focus_param_t * param)
{
	...
	if (my_node->is_for_session) {
		for (list = g_focus_node_list; list != NULL; list = list->next) {
			...
			if (!node) {
				/* skip */
			} else if (node != my_node && node->pid == my_node->pid && node->is_for_session && !node->is_for_watch && my_node->status & node->status) {
