...
struct mm_sound_focus_dbus_method{
	struct mm_sound_dbus_method_info info;
        dbus_method_handler handler;
};

struct mm_sound_focus_dbus_signal{
...
struct mm_sound_focus_dbus_method methods[METHOD_CALL_MAX] = {
...
struct mm_sound_focus_dbus_signal signals[SIGNAL_MAX] = {
	[SIGNAL_TEST] = {
		.info = {
			.name = "SignalTest1",
		},
...
int MMSoundMgrDbusInit(void)
{
	...
	if (_mm_sound_mgr_dbus_own_name(G_BUS_TYPE_SYSTEM, BUS_NAME_FOCUS_SERVER, &sound_server_owner_id) != MM_ERROR_NONE) {
