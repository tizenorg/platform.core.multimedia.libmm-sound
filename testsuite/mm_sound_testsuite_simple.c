...
void device_connected_cb(MMSoundDevice_t device_h, bool is_connected, void *user_data)
{
	...
	char *name = NULL;
	debug_log
		("*** device_connected_cb is called, device_h[%p], is_connected[%d], user_date[%p]\n",
...
void device_info_changed_cb(MMSoundDevice_t device_h, int changed_info_type, void *user_data)
{
	...
	char *name = NULL;
	debug_log
		("*** device_info_changed_cb is called, device_h[%p], changed_info_type[%d], user_date[%p]\n",
...
#ifdef USE_FOCUS
void focus_cb0(int index, mm_sound_focus_type_e type,
			   mm_sound_focus_state_e state, const char *reason_for_change, const char *additional_info, void *user_data)
{
	char *_state = NULL;
	if (state == FOCUS_IS_RELEASED)
		...
		_state = "ACQUIRED";
	debug_log
		("*** focus_cb0 is called, index[%d], focus_type[%d], state[%s], reason_for_change[%s], additional_info[%s], user_data[%s]\n",
		 index, type, _state, reason_for_change, additional_info, (char *)user_data);
}

void focus_cb1(int index, mm_sound_focus_type_e type,
			   mm_sound_focus_state_e state, const char *reason_for_change, const char *additional_info, void *user_data)
{
	char *_state = NULL;
	if (state == FOCUS_IS_RELEASED)
		...
		_state = "ACQUIRED";
	debug_log
		("*** focus_cb1 is called, index[%d], focus_type[%d], state[%s], reason_for_change[%s], additional_info[%s], user_data[%s]\n",
		 index, type, _state, reason_for_change, additional_info, (char *)user_data);
}

void focus_watch_cb(int index, mm_sound_focus_type_e type,
					mm_sound_focus_state_e state, const char *reason_for_change, const char *additional_info, void *user_data)
{
	char *_state = NULL;
	if (state == FOCUS_IS_RELEASED)
		...
		_state = "ACQUIRED";
	debug_log
		("*** focus_watch_cb is called, index[%d], focus_type[%d], state[%s], reason_for_change[%s], additional_info[%s], user_data[%s]\n",
		 index, type, _state, reason_for_change, additional_info, (char *)user_data);
}
#endif
void quit_program()
...
{
	switch (route) {
	case MM_SOUND_ROUTE_OUT_SPEAKER:
		return "OUT_SPEAKER";
	case MM_SOUND_ROUTE_OUT_RECEIVER:
		return "OUT_RECEIVER";
	case MM_SOUND_ROUTE_OUT_WIRED_ACCESSORY:
		return "OUT_WIRED_ACCESSORY";
	case MM_SOUND_ROUTE_OUT_BLUETOOTH_A2DP:
		return "OUT_BLUETOOTH_A2DP";
	case MM_SOUND_ROUTE_OUT_DOCK:
		return "OUT_DOCK";
	case MM_SOUND_ROUTE_OUT_HDMI:
		return "OUT_HDMI";
	case MM_SOUND_ROUTE_OUT_MIRRORING:
		return "OUT_MIRRORING";
	case MM_SOUND_ROUTE_OUT_USB_AUDIO:
		return "OUT_USB";
	case MM_SOUND_ROUTE_OUT_MULTIMEDIA_DOCK:
		return "OUT_MULTIMEDIA_DOCK";
	case MM_SOUND_ROUTE_IN_MIC:
		return "IN_MIC";
	case MM_SOUND_ROUTE_IN_WIRED_ACCESSORY:
		return "IN_WIRED_ACCESSORY";
	case MM_SOUND_ROUTE_IN_MIC_OUT_RECEIVER:
		return "IN_MIC_OUT_RECEIVER";
	case MM_SOUND_ROUTE_IN_MIC_OUT_SPEAKER:
		return "IN_MIC_OUT_SPEAKER";
	case MM_SOUND_ROUTE_IN_MIC_OUT_HEADPHONE:
		return "IN_MIC_OUT_HEADPHONE";
	case MM_SOUND_ROUTE_INOUT_HEADSET:
		return "INOUT_HEADSET";
	case MM_SOUND_ROUTE_INOUT_BLUETOOTH:
		return "INOUT_BLUETOOTH";
	default:
		return NULL;
	}
}
...
static void interpret(char *cmd)
{
	...
	case CURRENT_STATUS_MAINMENU:
		...
		if (strncmp(cmd, "DS", 2) == 0) {
			ret =
				mm_sound_subscribe_signal
				(MM_SOUND_SIGNAL_RELEASE_INTERNAL_FOCUS, &g_subscribe_id1, __mm_sound_signal_cb1, NULL);
			if (ret < 0)
			...
			ret =
				mm_sound_subscribe_signal
				(MM_SOUND_SIGNAL_RELEASE_INTERNAL_FOCUS, &g_subscribe_id2, __mm_sound_signal_cb2, NULL);
			if (ret < 0)
				debug_log("mm_sound_subscribe_signal() failed with 0x%x\n", ret);
			else
			...
			if (ret < 0)
				...
				debug_log
					("mm_sound_send_signal for signal[%d], value[%d] is success\n", MM_SOUND_SIGNAL_RELEASE_INTERNAL_FOCUS, 1);
			...
			ret =
				mm_sound_register_focus_for_session(id, getpid(), stream_type,
													(id == 0) ? focus_cb0 : focus_cb1, (void *)user_data);
			if (ret) {
			...
			ret =
				mm_sound_set_focus_watch_callback_for_session(getpid(), type,
															  focus_watch_cb, (void *)user_data, &g_focus_watch_index);
			if (ret) {
...
#endif
		{
		...
		} else if (strncmp(cmd, "q", 1) == 0) {	//get media volume
		...
		} else if (strncmp(cmd, "e", 1) == 0) {	//set media volume down
			unsigned int value = 100;
			ret = mm_sound_volume_get_value(g_volume_type, &value);
			if (ret < 0) {
				debug_log("mm_sound_volume_get_value 0x%x\n", ret);
		...
		} else if (strncmp(cmd, "r", 1) == 0) {	//get media volume
			unsigned int value = 100;
		...
		} else if (strncmp(cmd, "y", 1) == 0) {	//set media volume down
			unsigned int value = 100;
			ret = mm_sound_volume_get_value(VOLUME_TYPE_SYSTEM, &value);
			if (ret < 0) {
				debug_log("mm_sound_volume_get_value 0x%x\n", ret);
		...
		} else if (strncmp(cmd, "g", 1) == 0) {	//get voice volume
			unsigned int value = 100;
		...
		} else if (strncmp(cmd, "j", 1) == 0) {	//set voice volume down
			unsigned int value = 100;
			ret = mm_sound_volume_get_value(VOLUME_TYPE_VOICE, &value);
			if (ret < 0) {
				debug_log("mm_sound_volume_get_value 0x%x\n", ret);
			...
			while (num != 'q') {
				fflush(stdin);
				g_print
					("enter number(0~H exit:q), volume type(0~7),  volume(0.0~1.0),  time(ms), enable_session(0:unable , 1:enable):\t ");
				...
				tok = strtok(NULL, " ");
				if (tok)
					enable_session = (bool) atof(tok);

				debug_log
					("volume type: %d\t volume is %f\t tonetime: %d\t enable_session %d \n",
				...
				while ((entry = readdir(basedir)) != NULL) {
					...
					while ((test_callback_done == 0) && (playfail == 0)) {
						...
						if (current_time.tv_sec - start_time.tv_sec > 200) {
							if ((++mywait) % 5 == 0) {
								debug_log
									("I'm waiting callback for %d seconds after play %s\n",
	...
	if (strncmp(cmd, "z", 1) == 0) {
		...
		if (ret) {
			...
			do {
				...
				if (dret) {
					...
					debug_log
						("*** --- [NEXT DEVICE] type[%d], id[%d], io_direction[%d], state[%d], name[%s]\n",
						 device_type, id, io_direction, state, name);
				}
			...
			do {
				...
				if (dret) {
					...
					debug_log
						("*** --- [PREV DEVICE] type[%d], id[%d], io_direction[%d], state[%d], name[%s]\n",
						 device_type, id, io_direction, state, name);
				}
			} while (dret == MM_ERROR_NONE);
		...
		ret =
			mm_sound_add_device_connected_callback(device_flag_1 | device_flag_2
												   | device_flag_3, device_connected_cb, NULL, &g_subs_id_device_conn);
		if (ret) {
		...
		ret =
			mm_sound_add_device_information_changed_callback(device_flag_1 |
															 device_flag_2 |
															 device_flag_3,
															 device_info_changed_cb, NULL, &g_subs_id_device_info);
		if (ret) {
