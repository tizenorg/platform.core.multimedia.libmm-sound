...
void focus_cb0 (int index, mm_sound_focus_type_e type, mm_sound_focus_state_e state, const char *reason_for_change, const char *additional_info, void *user_data)
{
	...
	if (state == FOCUS_IS_RELEASED) {
		_state = "RELEASED";
		ret = mm_sound_disable_focus_reacquirement(index, true);
		if (!ret)
			debug_log("disable fcous reacquirement success");
	} else
		_state = "ACQUIRED";
	...
	switch (g_menu_state)
	{
		case CURRENT_STATUS_MAINMENU:
			...
			if(strncmp(cmd, "DS", 2) == 0) {
				...
				char flag_1, flag_2;
				int id = 0;
				bool disable_reacquirement = false;
				fflush(stdin);
				g_print ("1. disable auto reacquirement\n");
				g_print ("2. inable auto reacquirement\n");
				...
				if(flag_2 == '1') { disable_reacquirement = true; }
				else if(flag_2 == '2') { disable_reacquirement = false; }
				else { disable_reacquirement = false; }
				ret = mm_sound_disable_focus_reacquirement(id, disable_reacquirement);
				if (ret) {
