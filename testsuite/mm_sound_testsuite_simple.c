	...
	switch (g_menu_state)
	{
		case CURRENT_STATUS_MAINMENU:
...
#endif
			{
			    ...
			    ret = mm_sound_remove_test_callback(subs_id[0]);
			    if (ret) {
		...
		else if(strncmp(cmd, "}", 1) == 0)
		{
			...
			int ret = 0;
			ret = mm_sound_remove_device_connected_callback(subs_id[1]);
			if (ret) {
			...
			if (ret) {
				...
				g_print("device_flags[0x%x], callback fun[%p], subs_id[%d]\n", device_flag_1|device_flag_2|device_flag_3, device_info_changed_cb, subs_id[2]);
			}
		}
			...
			int ret = 0;
			g_print("subs_id = %d\n", subs_id[2]);
			ret = mm_sound_remove_device_information_changed_callback(subs_id[2]);
			if (ret) {
